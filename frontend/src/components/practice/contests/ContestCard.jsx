...
const ContestCard = (props) => {
  ...
  useState(()=>{
    const fetchContestProblems = async () => {
      try {
        const response = await apiService.getContestProblems(props.contest.id,'voyager_08');
        let temp=[];
    ...
    <div className="c-card">
      ...
      <Stack direction="horizontal" className="c-problems-container" gap={1}>
      {
        problems.map((problem)=>{
            return <Badge bg={problem.bg} pindex={problem.index} className="p-badge" onClick={handleProblem}>{problem.index}</Badge>
        })
      }
    </Stack>
