...
router.post("/create", verifyJwtToken, async (req, res) => {
  ...
  try {
    ...
    if (rows.length == 1) {
      ...
      connection.commit();
      const [post] = await connection.promise().query(`SELECT * FROM postDetails WHERE postid='${postid}'`);
      return res.status(200).json({ message: "post created successfully", data:post[0] });
    }
