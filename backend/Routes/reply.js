...
router.post("/create", verifyJwtToken, async (req, res) => {
  ...
  try {
    ...
    console.log("users with given username and usertype in create reply ", rows);
    ...
    if (rows.length == 1) {
      ...
      const [reply] = await connection.promise().query(`SELECT * FROM replyDetails WHERE replyid='${replyid}'`);

      return res.status(200).json({ message: "reply created", data: reply[0] });
    }
