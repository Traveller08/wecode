...
const get_color =(rating) =>{
    ...
    if (rating <= 1199)
        col = '#cec8c1'
    else if (1199 < rating <= 1399)
        col = '#43A217'
    else if (1399 < rating <= 1599)
        col = "#22C4AE"
    else if (1599 < rating <= 1899)
        col = "#1427B2"
    else if (1899 < rating <= 2099)
        col = "#700CB0"
    else if (2099 < rating <= 2299)
        col = "#F9A908"
    else if (2299 < rating <= 2399)
        col = "#FBB948"
    else
        col = "#FF0000"
    return col
...
//   console.log("req query ", req.url);
  ...
  try {
    const problems = cf_problems.filter((problem)=>{
        if(combineByOr){
            for (let i in tags){
                if(problem.tags.includes(tags[i]) && problem.rating>=from && problem.rating<=to){     
                    return true;
                }
            }
            return false;
    ...
    return res.status(200).json({message:"problems fetched successfully", data:problems})
  } catch (error) {
...
router.get("/contests", async (req, res) => {
    ...
    // console.log("here in contests ", contestType)
    try {
        let contests
        if (contestType==="All contests"){
            ...
            contests = cf_contests.filter((contest)=>{return contest.name.includes(contestType) && !contest.name.includes("Educational") && !contest.name.includes("Div. 1 + Div. 2")})
        }else{

            contests = cf_contests.filter((contest)=>{return contest.name.includes(contestType)})
        }
        //  console.log("contests")
      return res.status(200).json({message:"contests fetched successfully", data:contests})
    } catch (error) {
...
router.get("/contest/problems", async (req, res) => {
    ...
    try {
        ...
        if(cf_username){
          ...
          const response = await cfapi.get(`/contest.status?handle=${cf_username}&contestId=${contestId}&from=1&count=1000`);
      ...
      return res.status(200).json({message:"problems fetched successfully", data:{problems:contest_problems,verdicts:final_verdicts}});
    } catch (error) {
